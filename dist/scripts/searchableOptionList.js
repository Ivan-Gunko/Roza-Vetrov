        ...
        DEFAULT_OPTIONS = {
            ...
            onScroll: function (sol) {
                ...
                sol.selectionContainer
                    ...
                    .css('width', this.settings.maxWidth
                                        ? this.settings.maxWidth
                                        : Math.ceil((sol.input.outerWidth() + 20) * 1.2));
            },
            onShown: null,
            onHidden: null,
            onChange: null
        ...
        renderOption: function (index, item, $container) {
            ...
            if (this.useCheckboxes) {
                ...
                $uiInput = $('<input type="radio" class="sol-radio" />')
                    .on('change', function () {
                        sol.selectionContainer.find('input[type="radio"][name="' + inputName + '"]').not($(this)).trigger('sol-deselect').closest('.sol-input-container');
                    })
                    ...
                    .on('click', function() {
                        ...
                        $('<input type="text" />').attr({placeholder: 'value1'});
 
                    });

                    if (this.settings.radioDisable) {$uiInput.hide()};
            }
        ...
        addSelectionDisplayItem: function (item, $uiInput) {
            ...
            if ($existingDisplayItem.length === 0) {
                var $displaySelectionItem = $('<div class="sol-selected-display-item" data-sol-item-val="' + $uiInput.val() + '" />')
                    ...
                    if (this.settings.radioDisable) {
                        $displaySelectionItem.hide();
                        this.input.attr({placeholder: this.showSelectionContainer.text()})
                    }
